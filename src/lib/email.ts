...
const FROM_EMAIL = 'The Curator <curator@inmysolitude.app>';
...
export async function sendVaultRequestReceivedEmail(to: string, bookTitle: string) {
  const html = buildEmailHtml(
    ...
    <p>I have received your petition to withdraw <strong>"${escapeHtml(bookTitle)}"</strong> from the Vault. Restricted materials require a careful evaluation of intent.</p>
  ...
  return resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: `Petition Received: ${escapeHtml(bookTitle)}`,
    html,
  });
}
...
export async function sendVaultRequestApprovedEmail(to: string, bookTitle: string, adminNote?: string) {
  let notesHtml = '';
  if (adminNote) {
    notesHtml = `<div class="note"><strong>Curator's Note:</strong> ${escapeHtml(adminNote)}</div>`;
  }
  ...
  const html = buildEmailHtml(
    ...
    <p>You have been granted access to <strong>"${escapeHtml(bookTitle)}"</strong>. A private reading room has been allocated for your study of this material.</p>
    ${notesHtml}
  ...
  return resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: `Access Granted: ${escapeHtml(bookTitle)}`,
    html,
  });
}
...
export async function sendVaultRequestDeniedEmail(to: string, bookTitle: string, adminNote?: string) {
  ...
  const html = buildEmailHtml(
    ...
    <p>I regret to inform you that access to <strong>"${escapeHtml(bookTitle)}"</strong> cannot be granted at this time. Such materials bear certain psychological weights that we must guard against.</p>
    ${notesHtml}
  ...
  return resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: `Access Denied: ${escapeHtml(bookTitle)}`,
    html,
  });
}
